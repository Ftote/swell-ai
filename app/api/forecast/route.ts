...
export async function GET(request: Request) {
  ...
  // Accept per-spot coordinates; default to central Bali
  const { searchParams } = new URL(request.url);
  const lat = parseFloat(searchParams.get("lat") ?? "-8.85");
  const lng = parseFloat(searchParams.get("lng") ?? "115.08");
  ...
  try {
    const res = await fetch(url, {
      ...
      // Cache 24h per spot — avoids exhausting the 10 req/day free limit
